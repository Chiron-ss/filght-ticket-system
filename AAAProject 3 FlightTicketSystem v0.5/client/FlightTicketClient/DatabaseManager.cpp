...
{
    if (!isConnected()) {
        qCritical() << "数据库未连接，无法更新航班";
        return false;
    }

    QSqlQuery query(m_db);
    // 关键修复：SQL字段名改为下划线格式（和addFlight一致）
    query.prepare(R"(
        UPDATE flights
        SET flight_number = :flightNum,
            origin = :origin,
            destination = :dest,
            ...
            total_economy_seats = :ecoTotal,
            available_economy_seats = :ecoAvail,
            total_business_seats = :busTotal,
            available_business_seats = :busAvail,
            aircraft_model = :aircraft,
            has_meal = :hasMeal
        WHERE id = :flightId
    )");

    // 绑定参数（无需修改，参数名和Flight结构体对齐即可）
    query.bindValue(":flightId", flight.id);
    query.bindValue(":flightNum", flight.flightNumber);
    query.bindValue(":origin", flight.origin);
    query.bindValue(":dest", flight.destination);
    query.bindValue(":departTime", flight.departureTime);
    query.bindValue(":arriveTime", flight.arrivalTime);
    query.bindValue(":airline", flight.airline);
    query.bindValue(":ecoPrice", flight.economyPrice);
    query.bindValue(":busPrice", flight.businessPrice);
    query.bindValue(":ecoTotal", flight.totalEconomySeats);
    query.bindValue(":ecoAvail", flight.availableEconomySeats);
    query.bindValue(":busTotal", flight.totalBusinessSeats);
    query.bindValue(":busAvail", flight.availableBusinessSeats);
    query.bindValue(":aircraft", flight.aircraftModel);
    query.bindValue(":hasMeal", flight.hasMeal);

    // 打印错误信息（方便排查）
    if (!query.exec()) {
        qCritical() << "更新航班SQL执行失败：" << query.lastError().text();
    ...
    if (query.numRowsAffected() == 0) {
        qCritical() << "更新航班失败：未找到ID为" << flight.id << "的航班";
        return false;
    }
